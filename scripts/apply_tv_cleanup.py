...
def remove_functions_containing_cast(text: str) -> str:
    """Remove complete Kotlin functions whose declaration is part of Cast UI/playback."""
    ...
    while i < len(lines):
        ...
        if function_re.match(line) or function_re2.match(line):
            ...
            i += 1
            # Kotlin functions can put the opening brace on a later line.
...
# Remove complete Cast methods first, then the remaining Cast-specific fields/calls.
for p in ROOT.glob('app/src/main/**/*.kt'):
    s = p.read_text()
    old = s
    s = remove_functions_containing_cast(s)

    # Constructor parameters / injected fields / state declarations tied to Cast.
    ...
    s = re.sub(r'^\s*\w*cast\w*\s*:\s*[^,\n]+,?\s*\n', '', s, flags=re.I | re.M)
    s = re.sub(r'^\s*\w*Cast\w*\s*:\s*[^,\n]+,?\s*\n', '', s, flags=re.M)

    # Remove standalone Cast state/flow/event lines and Cast calls left outside removed methods.
    ...
    # Remove one-line Cast expressions/imports that may remain in UI code.
...
s = re.sub(
    r'fun settings\(backupUri: String\? = null\) = Routes\.SETTINGS_DESTINATION\.replace\("\{backupUri\}", Uri\.encode\(backupUri\)\)',
