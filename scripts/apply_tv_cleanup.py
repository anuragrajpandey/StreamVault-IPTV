...
for p in ROOT.glob('app/src/main/**/*.kt'):
    ...
    # Remove common standalone feature calls/fields that are safe to delete line-wise.
    s=re.sub(r'^\s*android\.Manifest\.permission\.POST_NOTIFICATIONS.*\n','',s,flags=re.M)
...
# Delete PiP/record/cast/download controls from player overlay source files by deleting their small declarations/calls.
# This intentionally targets named feature symbols, not generic words like "record" or "download".
for p in (APP/'ui/screens/player').rglob('*.kt'):
    ...
    # Remove obvious private state lines containing dedicated coordinator/property names.
    s=re.sub(r'^\s*(private\s+)?(val|var)\s+\w*(cast|Cast|recording|Recording|pictureInPicture|PictureInPicture)\w*\s*=.*\n','',s,flags=re.M)
    if s!=old: p.write_text(s)

# Remove PiP and cast Gradle dependencies by exact artifact identifiers if present.
