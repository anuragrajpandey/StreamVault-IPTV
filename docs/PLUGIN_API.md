# EliteStocks TV Plugin API

EliteStocks TV plugins are companion Android APKs. They do not inject code into the
main application. Instead, EliteStocks TV discovers installed APKs that expose a
...
EliteStocks TV from a declarative schema, or opened as a native plugin Activity when
...
- Any manual Android package install. EliteStocks TV detects it after refreshing the
  Plugins screen.

## Manifest

EliteStocks TV first asks the plugin service for its manifest. As a fallback, it
...
- `provider.m3u`: the plugin can expose an M3U URL that EliteStocks TV imports as a
...
- `configuration.schema`: the plugin exposes a declarative configuration schema
  that EliteStocks TV renders with its own UI.
- `configuration.activity`: the plugin exposes a native Android Activity that
  EliteStocks TV opens for configuration.
...
`handled=false` when the URL is not theirs. EliteStocks TV then continues with other
enabled plugins or the original URL.

`MSG_REWRITE_CAST_URL` is Cast-specific. EliteStocks TV sends the stream context that
...
`MSG_PREPARE_PLAYBACK` may also enrich the stream that EliteStocks TV sends to the
player. When `handled=true` and `success=true`, the plugin can return:

- `output_url`: final playable URL. If omitted, EliteStocks TV keeps `input_url`.
...
For `SMOOTH_STREAMING` + `clearkey`, EliteStocks TV normalizes the ISML
...
simple actions. EliteStocks TV owns the visual shell, validation placement, focus
...
EliteStocks TV owns the visual implementation.
...
For `MSG_SET_CONFIGURATION_VALUES`, EliteStocks TV sends the same JSON object in
...
For `MSG_RUN_CONFIGURATION_ACTION`, EliteStocks TV sends `configuration_action_id`.
...
EliteStocks TV starts `configurationActivityAction` with the plugin package set
...
Native plugin Activities should feel at home when launched from EliteStocks TV and
...
- Design TV-first: every important control must be D-pad focusable and readable
  from couch distance.
- Keep the style restrained and close to EliteStocks TV: dark background, compact
...
- Test both entry points: launched directly from the plugin APK and launched from
  EliteStocks TV's Plugins screen.
- Test on the same device classes EliteStocks TV supports, including TV devices such
